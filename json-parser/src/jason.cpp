...
void Jason::processTokens() {  // processes the tokens to form a valid json
    ...
    for (const auto& token : tokens) {
        ...
        cout << "Current state: ";
        switch(state) {
            // When starting, look for the start of any primitive
            ...
            case JasonState::OpenCurlyBracket: {
                ...
                switch (currTokenType) {
                    case TokenType::ClosedCurlyBracket: {
                        ...
                        state = (recursiveState.empty()) ? JasonState::Finished : JasonState::PostValue;
                        break;                                                                                  
                    }
                    case TokenType::String: {
            ...
            // After a colon, we begin searching for a value, there are 6 valid values
            ...
            // brackets
            case JasonState::PostValue: {  // TODO: This entire section needs to be verified
                cout << "postvalue\n";
            ...
            // This means we can look for any of the valids values (including other arrays and objects) or a closing bracket
            case JasonState::OpenSquareBracket: {
            ...
            // TODO: May need to reimplement the closing curly bracket
            ...
            // When in an array, we search for the next value (can be any valid value type, including nested arrays/objects), anything else is invalid
            ...
            case JasonState::Comma: { 
                ...
                switch (prevState) {
                    case JasonState::OpenCurlyBracket: {
                        cout << "comma: previous state is curly bracket\n";
                        state = (currTokenType == TokenType::String) ? JasonState::Key : JasonState::Invalid;
                        break;
                    }
                    case JasonState::OpenSquareBracket: {
                        cout << "comma: previous state is square bracket\n";
                        switch(currTokenType) {
                            case TokenType::OpenCurlyBracket: {
                                recursiveState.push(JasonState::OpenCurlyBracket);
                                state = JasonState::OpenCurlyBracket;
